import { useWalletConnection, useSendTransaction, useTransactionPool, useWalletSession } from "@solana/react-hooks";
...
export function useCreditScore() {
    ...
    const [cachedIdentityPda, setCachedIdentityPda] = useState<Address | null>(null);
    ...
    const calculateScore = useCallback(async () => {
        ...
        setCalculating(true);
        try {
            const logStyle = "background:#fff3cd;color:#664d03;font-weight:600;padding:2px 6px;border-radius:4px;border:1px solid #ffeeba";
            ...
            console.log("%c[CreditScore] 交易发送耗时: " + (performance.now() - tSendStart).toFixed(0) + "ms", logStyle);
            return true;
        } catch (error) {
