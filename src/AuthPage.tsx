  ...
  if (status === "connected") {
    ...
    return (
      <div className={`${theme.layout.pageContainer} text-center`}>
        ...
        <div className={`${theme.layout.cardCompact} max-w-md mx-auto space-y-2 text-left`}>
            <div>
        ...
        {!exists && (
            <div>
                <button onClick={() => onNavigate("identity")} className={theme.typography.link}>
        ...
        <div className="pt-8">
            <button onClick={() => disconnect()} className={`${theme.typography.error} cursor-pointer hover:text-red-600`}>
                Disconnect Wallet
            </button>
        </div>
        ...
        <div className="text-xs text-muted pt-4 border-t border-border-low mt-4 flex flex-col gap-2 items-center">
            ...
            <div className="flex items-center gap-2">
                <div className={`${theme.status.badge} ${rpcConnectionStatus === 'ok' ? theme.status.verified : rpcConnectionStatus === 'error' ? theme.status.error : theme.status.unverified}`}></div>
